  ...
  return (
    <div>
      <Card>
        <Card>
          <Label>반복 횟수</Label>
          <Input>[여긴 추후 수정, select로 선택 혹은 input으로 임력]</Input>
        </Card>
        <Card>
          <Label>변수 설정</Label>
          {variables.map((v, idx) => (
          ...
          <Button onClick={() => onVariableAddClick()}>변수 추가</Button>
        </Card>
        <Card>
          <Input>[여긴 sequence 및 separator 설정하는 부분, 변수는 반복횟수처럼 해둬야할거고..]</Input>
        </Card>
      </Card>
    </div>
  )
